...
export function UsagePage() {
    ...
    return (
      <div className="space-y-6">
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground font-medium mb-4">Unable to load usage data</p>
  ...
  const criticalMetrics = metrics.filter(m => m.limit && (m.used / m.limit) >= 0.9);
  const warningMetrics = metrics.filter(m => m.limit && (m.used / m.limit) >= 0.75 && (m.used / m.limit) < 0.9);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Monitor your {planDetails.name} plan usage and manage your subscription
            </p>
          </div>
              ...
              <>
                {criticalMetrics.length > 0 && (
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-destructive/10 border border-destructive/20 rounded-full">
                    ...
                    <span className="text-xs font-medium text-destructive">Limits Reached</span>
                  </div>
                )}
                {warningMetrics.length > 0 && criticalMetrics.length === 0 && (
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-yellow-50 border border-yellow-200 rounded-full">
                    ...
                    <span className="text-xs font-medium text-yellow-700">Approaching Limits</span>
                  </div>
                )}
              </>
            )}
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="flex items-baseline gap-2">
                  <div className="text-4xl font-bold text-foreground">${planDetails.price_monthly}</div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-medium text-foreground">
                  {criticalMetrics.length > 0 ? 'Action Required' : 'Consider Upgrading'}
                </h3>
