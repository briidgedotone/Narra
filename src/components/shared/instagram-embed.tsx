...
function InstagramEmbedComponent({
  ...
  // Memoized embed HTML generation - only regenerate when URL changes
...
<script async src="//www.instagram.com/embed.js"></script>`;
  ...
  // Optimized Instagram embed processing
  useEffect(() => {
    // Only process if URL is valid and has changed, or first mount
    if (!isValidUrl || !embedHtml) return;
    
    const urlChanged = urlRef.current !== url;
    
    if (!hasProcessedRef.current || urlChanged) {
      urlRef.current = url;
      hasProcessedRef.current = true;
      
      // Process Instagram embeds with slight delay to ensure DOM is ready
      const processEmbeds = () => {
