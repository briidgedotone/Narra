...
// TikTok + Instagram BulkScrapeProcessor class
class BulkScrapeProcessor {
  ...
  private async fetchIndividualPost(url: string): Promise<any> {
    try {
      const { scrapeCreatorsApi, transformers } = await import('@/lib/api/scrape-creators');
      ...
      if (isTikTok) {
        ...
        if (!response.success) {
          return {
            success: false,
            error: response.error || 'Failed to fetch TikTok video data',
        ...
        if (!transformedData) {
          return {
            success: false,
            error: 'Failed to transform TikTok video data'
          };
        }

        // Update embedUrl with original URL if not set
        if (!transformedData.embedUrl) {
          transformedData.embedUrl = url;
          transformedData.originalUrl = url;
        }
        ...
        if (!response.success) {
          return {
            success: false,
            error: response.error || 'Failed to fetch Instagram post data',
            cached: response.cached 
          };
        }
        ...
        if (!postData) {
          return {
            success: false,
            error: 'No Instagram post data found in response'
          };
        }
  ...
  private transformToSavePostData(postResponse: any): any {
    ...
    const platform = postResponse.platform;
    
    if (platform === 'tiktok') {
      // TikTok data is already in the correct format from transformer
      return {
        handle: post.handle,
  ...
  private normalizePlatformPostId(platformPostId: string, embedUrl: string, platform: string): string {
  ...
  private async savePostToBoard(postData: any): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const normalizedPlatformPostId = this.normalizePlatformPostId(
      ...
      // Use minimal post data structure to avoid schema issues
  ...
  async processUrls(urls: string[]): Promise<void> {
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      try {
        ...
        const platform = postResponse.platform === 'tiktok' ? 'TikTok' : 'Instagram';
        const username = postResponse.platform === 'tiktok' 
          ? postResponse.data?.transformed.handle
          : `@${postResponse.data?.transformed.owner?.username}`;
          
        console.log(`  ✅ Fetched ${platform} data for ${username}`);
...
// Main
async function main() {
  console.log('🎯 Best of SaaS Apps TikTok Bulk Scraper');
  console.log('=====================================\n');
  ...
  try {
    ...
    // Filter to only TikTok URLs (lines 1-70)
    const allUrls = fileContent.split('\n')
      .map(line => line.trim())
      .filter(line => line.length > 0);
    
    urls = allUrls.filter(url => url.includes('tiktok.com'));
    
    console.log(`📋 Found ${urls.length} TikTok URLs to process`);
    console.log(`📊 Total URLs in file: ${allUrls.length}`);
    console.log('🎵 Processing only TikTok URLs\n');
  } catch (error) {
