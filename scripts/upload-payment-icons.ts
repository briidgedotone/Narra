#!/usr/bin/env tsx
/**
 * Upload Payment Icons Script
 * 
 * This script uploads the green checkmark icon for payment success emails
...
async function updateEmailImagesConfig(successIconUrl: string) {
  log('\n📝 Updating email-images.ts config...', colors.cyan);
  
  const configContent = `// Email image URLs from Supabase storage
// Auto-generated by upload-payment-icons.ts script
...
`;
  ...
  try {
    ...
    log(`✅ Updated email-images.ts with success icon`, colors.green);
  } catch (error) {
...
async function main() {
  log('🎯 Uploading payment success icon...', colors.blue);
  
  // You need to provide the path to the green checkmark image
  // For now, I'll use a placeholder that shows what the user needs to do
  const tempImagePath = process.argv[2];
  
  if (!tempImagePath) {
    log('❌ Please provide the path to the green checkmark image:', colors.red);
    log('   npx tsx scripts/upload-payment-icons.ts /path/to/checkmark.png', colors.yellow);
    return;
  }
  ...
  try {
    ...
    // Upload the success icon
    const successIconUrl = await uploadImageFromTempPath(tempImagePath, 'email-assets', 'success-icon.png');
    
    if (successIconUrl) {
      await updateEmailImagesConfig(successIconUrl);
      
      log('\n🎉 Success! Next steps:', colors.green);
      log('1. Update payment-success-email.tsx to use EMAIL_IMAGES.SUCCESS_ICON', colors.cyan);
